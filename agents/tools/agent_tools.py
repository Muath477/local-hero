...
DELEGATE_TOOLS = ("ask_writer_agent", "ask_coder_agent", "ask_researcher_agent", "ask_tools_agent")
...
@tool(
    "Delegate a sub-task that needs a real tool to the tools agent: searching the web "
    "(news, prices, weather, anything current), calculations, the current date and time, "
    "listing uploaded files, translating text or documents. Use it whenever the answer "
    "depends on live data or an exact computation — never answer those yourself."
)
def ask_tools_agent(task: str) -> str:
    return _delegate("tool_caller", task)
